...
const getCheckboxes = document.querySelectorAll('.spectrum-Table-checkboxCell, .spectrum-Table-checkbox, .spectrum-Table-cell[aria-colindex="4"], .spectrum-Table-cell[aria-colindex="3"]');
...
Array.from(getCheckboxes).forEach(
  (el) => {
    el.onclick = () => {
      ...
      //apply select styles
      if (document.querySelector('.Container-rRail').style.maxWidth === "300px") {
        document.querySelector('.Container-rRail').style = 'max-width:0;border:none;';
        window.dispatchEvent(new Event('resize'));
      } else {
        document.querySelector('.Container-rRail').style = 'max-width:300px;border-left:1px solid #c1c1c1;padding: 12px';
      ...
      if (el.parentElement.querySelector('input')) {
        if (el.parentElement.querySelector('input').checked === true) {
          ...
          // let countChecks = //create a counter for multiple selections
